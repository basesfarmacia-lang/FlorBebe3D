...
// Variables para el analizador
let audioCtx, analyser, dataArray, source;

startBtn.addEventListener('click', () => {
    // 1. Iniciar el AudioContext (obligatorio tras interacción del usuario)
    if (!audioCtx) {
    ...
    // 2. Si el contexto estaba suspendido (común en Chrome), lo reanudamos
    if (audioCtx.state === 'suspended') {
        audioCtx.resume();
    }

    // 3. Reproducir la canción completa
    audio.play().catch(e => console.error("Error al reproducir:", e));

    // 4. (Opcional) Ocultar el botón y mostrar la flor
    container.style.display = 'none'; 
